            ...
            if (currentWallet) {
              ...
              if (orderUpdated) {
                ...
                try {
                  ...
                  session.endSession();
                  console.error("⚠️ Transaction failed in ShreeMarutiWebhook cancellation:", err.message);
                }

                order.walletRefunded = true;
                order.status = "Cancelled";
              } else {
                console.log(`Shree Maruti Webhook: Order ${order.awb_number} was already refunded/cancelled elsewhere. Skipping refund.`);
                order.walletRefunded = true;
                order.status = "Cancelled";
              }
            }
